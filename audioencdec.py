...
class Main :
    ...
    def __init__(self) :
        ...
        with open(aud_url, "rb") as aud2string:
            converted_string = base64.b64encode(aud2string.read())


        with open('audenc.bin', "wb") as file:
                    file.write(converted_string)



        file = open('audenc.bin', 'rb')
        byte = file.read()
        data=str(byte)
        file.close()
        ...
        fc=open('cipheraud.mpeg','wb')
        res = bytes(enc, 'utf-8')
        fc.write(base64.b64decode((res)))
        fc.close()


        f=open('cipheraud.txt','w')
        f.write(enc)
        f.close()

        f=open('cipheraud.txt','r')
        data=f.read()
        f.close()
        t0=time.time()
