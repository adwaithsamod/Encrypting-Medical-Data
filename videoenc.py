...
class Main :
    ...
    def __init__(self) :
        ...
        vid_url = "VID-20220312-WA0000.mp4"
        # def binary_to_string(binary):
        #    return b64encode(binary).decode("utf-8")
        
        with open(vid_url, "rb") as vid2string:
            converted_string = base64.b64encode(vid2string.read())


        with open('videnc.bin', "wb") as file:
                    file.write(converted_string)



        file = open('videnc.bin', 'rb')
        byte = file.read()
        data=str(byte)
        file.close()
        ...
        t0=time.time()
        # enc=imgencdec.Main.encprocess(data)
        enc=self.encprocess(data)
        # enc=e(data)
        t1=time.time()
        print(t1-t0)

        fc=open('ciphervid.mp4','wb')
        res = bytes(enc, 'utf-8')
        fc.write(base64.b64decode((res)))
        fc.close()


        f=open('ciphervid.txt','w')
        f.write(enc)
        f.close()

        f=open('ciphervid.txt','r')
        data=f.read()
        f.close()
        t0=time.time()
